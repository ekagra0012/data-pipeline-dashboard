...
# Route all logging to stderr so stdout is clean report-only output
logging.basicConfig(
    level=logging.INFO,
...
def clean_customers(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _parse_signup(val, idx):
        ...
        if pd.isna(result) and pd.notna(val):
            logger.warning(
                "Unparseable signup_date at row %s: '%s'", idx, val
            )
        return result
...
def clean_orders(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # F-13: Amount imputation (product-level median, fallback to global)
    ...
    if len(unmapped):
        logger.warning(
            "Unmapped status values (→ 'pending'): %s", list(unmapped)
        )
    ...
    print(
        f"  Rows before: {rows_before} | "
        f"Rows after: {len(df)} | "
        f"Unrecoverable rows dropped: {unrecoverable_count}"
    )
    print(f"  Null counts before: {null_before}")
    print(f"  Null counts after:  {null_after}")
...
def _ensure_raw_data(input_dir: pathlib.Path) -> None:
    """Auto-generate raw CSVs if missing (zero-intervention guarantee)."""
    required = ["customers.csv", "orders.csv", "products.csv"]
    missing = [f for f in required if not (input_dir / f).exists()]
    if missing:
        logger.warning(
            "Missing raw files %s in '%s' — auto-generating...",
            missing,
            input_dir,
        )
        import subprocess
...
def main():
    ...
    parser.add_argument(
        "--input-dir", default="data/raw", help="Raw CSVs directory"
    )
    parser.add_argument(
